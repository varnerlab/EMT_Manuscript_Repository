...
In the absence of YREG1, most of the population failed to consistently to retain a stable epithelial state (Fig. \ref{fig:F4}D).
Conversely, YREG1 overexpression revealed an enhanced epithelial phenotype, while some inherently transformed cells moved towards a hybrid phenotype  (Fig. \ref{fig:F4}C).
Elevation of YREG1 repressed LEF-1 and TGF-$\beta$3 expression, thereby not allowing free $\beta$-catenin to form the $\beta$-catenin-LEF-1 complex which promotes mesenchymal gene expression, or SMAD activity following from autocrine TGF-$\beta$3 signaling. Taken together, we found that low YREG1 expression was necessary for producing a stabile the model with an epithelial phentoype that is inducable across multiple stimuli such as TGF-$\beta$1/2, TGF-$\beta$3 and SNAIl/SLUG transfection, as seen in the training objectives.
